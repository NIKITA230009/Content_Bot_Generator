...
async def on_publish(cq: CallbackQuery):
    content_id = int(cq.data.split(":")[1])
    await push_to_ready_stream(content_id)
    await cq.message.edit_reply_markup(reply_markup=status_kb("\u2705 Опубликовано"))
...
async def on_prompt(cq: CallbackQuery, state: FSMContext):
    ...
    prompt_msg = await cq.message.answer("Отправьте ответным сообщением ваш промпт для переработки", reply_markup=cancel_kb())
...
async def on_skip(cq: CallbackQuery):
    ...
    await cq.message.edit_reply_markup(reply_markup=status_kb("\u23ed Пропущено"))
...
async def on_custom_prompt(msg: Message, state: FSMContext, bot: Bot):
    ...
    new_text = await rewrite_with_custom_prompt(raw_text, msg.text)
